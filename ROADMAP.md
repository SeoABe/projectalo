...
- [x] 소스/단계별 건수 진단 로그(`main/rss/afterDedup/new`) — Vercel 런타임 로그로 0건 원인 추적
...
## 5. UX / 다듬기 (낮음) — ✅ 완료
- [x] 관리자 페이지에도 새로고침 버튼
- [x] 대시보드 `meta.date`·주차(WK) 동적 계산 → KST 기준
- [x] 메인 로딩 스피너, 헤더 우측 버튼/로그아웃 겹침 정리(헤더 우측 여백 + 모바일 주차배지 숨김)

---

## 6. 운영/관측 (낮음, 단일 사용자 기준 후순위)
- [x] 입력 검증(POST/PUT 바디) — urgents/cards/profiles/keywords 필수값 누락 시 400
